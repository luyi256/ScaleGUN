...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--model", type=str, default="CGU", help="[CGU/GIF/IDEA]."
    )
    ...
    logger.info(args)
    statistics_prefix = f"{args.analysis_path}/{args.dataset}/{name}_result/Batch_{args.num_batch_removes}_Num_{args.num_removes}_lam_{args.lam}_lr_{args.lr}_std_{args.std}_prop_{args.prop_step}_r_{args.r}"
    ...
    if args.train_mode == "binary":
        ...
        val_mask = data.val_mask
        test_mask = data.test_mask
        y_train = F.one_hot(data.y[data.train_mask],
    ...
    if args.model in ['IDEA', "GIF"]:
        args.require_grad = True
    else:
        args.require_grad = False
    ...
    logger.info(f"opt_grad_norm:{opt_grad_norm}")
        ...
        for i in range(args.num_batch_removes):
            ...
            if args.train_mode == "ovr":
                ...
                for k in range(y_train.size(1)):
                    ...
                    if args.model in ['IDEA', "GIF"]:
                        # grad here needs retain_graph
                        w_k = w_approx[:, k].clone(
                        ).detach().requires_grad_(True)
                        grad_old = lr_grad(
                            w_k, X_old_train, y_rem, args.lam, retain_graph=True)
                        grad_new = lr_grad(
                            w_k, X_rem, y_rem, args.lam, retain_graph=True)
                        w_approx[:, k] += approxi(
                            (grad_old, grad_old, grad_new), w_k, args)
                    else:
                        grad_old = lr_grad(
    ...
    grad_norm_approx[0, 0] += opt_grad_norm
    grad_norm_approx = grad_norm_approx[:, 0].cumsum(0)
    grad_norm_worst = grad_norm_worst[:, 0].cumsum(0)
    if args.compare_gnorm:
        np.savetxt(f"{statistics_prefix}_grad_norm_approx_{edge_idx_start}.txt",
                   grad_norm_approx.numpy())
        np.savetxt(f"{statistics_prefix}_grad_norm_real_{edge_idx_start}.txt",
                   grad_norm_real.numpy())
        np.savetxt(f"{statistics_prefix}_grad_norm_worst_{edge_idx_start}.txt",
                   grad_norm_worst.numpy())
    # torch.save(
    #     {
