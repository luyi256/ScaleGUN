...
namespace propagation
{
    ...
    double Instantgnn::GetResidueSum(Eigen::Ref<Eigen::VectorXd> res)
    {
        // cout<<"res:"<<res.rows()<<" "<<res.cols()<<endl;
        // Eigen::MatrixXd tmp_result;
        // if (res.rows() != dimension)
        //     tmp_result = res.transpose();
        // else
        //     tmp_result = res;
    ...
    double Instantgnn::init_push_graph(string path, string dataset, Eigen::Map<Eigen::MatrixXd> &X, Eigen::Map<Eigen::MatrixXi> &edge_index, uint _layer, double _r, const vector<double> &_weights, uint _num_threads, double rmax)
    {
        ...
        residue = new double **[dimension];
        for (uint i = 0; i < dimension; i++)
        {
            q[i] = new double *[layer];
            residue[i] = new double *[layer];
            for (uint j = 0; j < layer; j++)
            {
                q[i][j] = new double[num_nodes];
                memset(q[i][j], 0, sizeof(double) * num_nodes);
                residue[i][j] = new double[num_nodes];
                memset(residue[i][j], 0, sizeof(double) * num_nodes);
            }
        }
        ...
        for (ti = 1; ti <= dimension % num_threads; ti++)
        {
            ...
            threads.push_back(thread(&Instantgnn::Push, this, start, ends, rmax, Eigen::Ref<Eigen::MatrixXd>(X)));
        }
    ...
    int Instantgnn::RandomWalkMethod(uint times, Eigen::Ref<Eigen::MatrixXd> result)
    {
        ...
        for (ti = 1; ti <= dimension % num_threads; ti++)
        {
            start = ends;
            ends += ceil((double)dimension / num_threads);
            threads.push_back(thread(&Instantgnn::RandomWalk, this, times, start, ends, result));
        }
        for (; ti <= num_threads; ti++)
        {
            start = ends;
    ...
    void Instantgnn::RandomWalk(uint times, uint start, uint ends, Eigen::Ref<Eigen::MatrixXd> result)
    {
        auto feat = result;
        init_dsfmt();
    ...
    void Instantgnn::PushMethod(Eigen::Ref<Eigen::MatrixXd> result, double rmax)
    {
        // cout << feat << endl;
        ...
        //     start = ends;
        //     ends += ceil((double)dimension / num_threads);
        //     threads.push_back(thread(&Instantgnn::Push, this, start, ends, rmax, Eigen::Ref<Eigen::MatrixXd>(tmp_result)));
        // }
        // for (; ti <= num_threads; ti++)
        // {
        //     start = ends;
    ...
    double Instantgnn::UpdateEdges(const vector<pair<uint, uint>> &edgepairs, Eigen::Ref<Eigen::MatrixXd> result, uint update_num_threads, double rmax)
    {
        // cout << "update result:" << result.rows() << " " << result.cols() << endl;
        // cout << "update tmp_result:" << tmp_result.rows() << " " << tmp_result.cols() << endl;
        // for (uint i = 0; i < dimension; i++)
        // {
        //     for (uint j = 0; j < num_nodes; j++)
        //     {
        ...
        //         cout << (q[i][0][j] * 0.1 + q[i][1][j] * 0.09) * pow(deg[j], -r) << endl;
        //         cout << "***" << endl;
        //     }
        // }
        // cout<<"result shape:"<<result.rows()<<" "<<result.cols()<<endl;
        // cout<<"to del:"<<edgepairs[0].first<<" "<<edgepairs[0].second<<endl;
        Timer queryTimer;
        double time;
        time = queryTimer.get_operation_time();
        vector<thread> threads;
        ...
        for (ti = 1; ti <= dimension % update_num_threads; ti++)
        {
            ...
            threads.push_back(thread(&Instantgnn::UpdateEdgesOperator, this, start, ends, rmax, ref(affectNodes), ref(affectNeighbors), Eigen::Ref<Eigen::MatrixXd>(result)));
        }
    ...
    double Instantgnn::UpdateStruct(const vector<pair<uint, uint>> &edgepairs)
    {
        Timer queryTimer;
        double time;
        time = queryTimer.get_operation_time();
    ...
    double Instantgnn::UpdateStructNodes(const vector<uint> &nodes)
    {
        Timer queryTimer;
        double time;
        time = queryTimer.get_operation_time();
    ...
    double Instantgnn::UpdateNodes(const vector<uint> &nodes, Eigen::Ref<Eigen::MatrixXd> result, uint update_num_threads, double rmax)
    {
        Timer queryTimer;
        double time;
        time = queryTimer.get_operation_time();
    ...
    double Instantgnn::UpdateFeatures(const vector<uint> &nodes, Eigen::Ref<Eigen::MatrixXd> result, uint update_num_threads, double rmax)
    {
        Timer queryTimer;
        double time;
        time = queryTimer.get_operation_time();
        ...
        for (ti = 1; ti <= dimension % update_num_threads; ti++)
        {
            start = ends;
            ends += ceil((double)dimension / update_num_threads);
            threads.push_back(thread(&Instantgnn::UpdateFeatureOperator, this, start, ends, nodes, rmax, Eigen::Ref<Eigen::MatrixXd>(result)));
        }
        for (; ti <= update_num_threads; ti++)
        {
            start = ends;
    ...
    void Instantgnn::UpdateFeatureOperator(uint start, uint ends, const vector<uint> &nodes, double rmax, Eigen::Ref<Eigen::MatrixXd> result)
    {
        for (uint i = start; i < ends; i++)
        {
    ...
    void Instantgnn::UpdateEdgesOperator(uint start, uint ends, double rmax, vector<uint> &affectNodes, vector<vector<uint>> &affectNeighbors, Eigen::Ref<Eigen::MatrixXd> result)
    {
        for (uint i = start; i < ends; i++)
        {
            vector<uint> candidates;
    ...
    void Instantgnn::PushOperator(uint dim, double rmax, Eigen::Ref<Eigen::MatrixXd> result, vector<uint> &candidates, vector<bool> &next_indicator_candidates, vector<uint> &next_candidates, uint k)
    {
    ...
    void Instantgnn::Push(uint start, uint ends, double rmax, Eigen::Ref<Eigen::MatrixXd> result)
    {
    ...
    double Instantgnn::PowerMethod(Eigen::Ref<Eigen::MatrixXd> result)
    {
        // cout << "result:" << result.rows() << " " << result.cols() << endl;
        Timer queryTimer;
        double time;
        ...
        for (ti = 1; ti <= dimension % num_threads; ti++)
        {
            start = ends;
            ends += ceil((double)dimension / num_threads);
            threads.push_back(thread(&Instantgnn::Power, this, start, ends, Eigen::Ref<Eigen::MatrixXd>(result)));
        }
        for (; ti <= num_threads; ti++)
        {
            start = ends;
    ...
    void Instantgnn::Power(uint start, uint ends, Eigen::Ref<Eigen::MatrixXd> result)
    {
        auto feat = result;
        // cout<<"start:"<<start<<" ends:"<<ends<<endl;
        for (uint i = start; i < ends; i++)
    ...
    uint Instantgnn::GetDegree(uint node)
    {
        return deg[node];
    }

    uint Instantgnn::GetNeighbor(uint node, uint index)
    {
