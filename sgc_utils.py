...
def lr_grad(w, X, y, lam, retain_graph=False):
    '''
    ...
    if not retain_graph:
        gradient = -X.t().mv((1 - z) * y) + lam * w
    else:
        loss = ovr_lr_loss(w, X, y, lam)
        gradient = grad(loss, w, create_graph=True, retain_graph=True)
    return gradient
...
def approxi(res_tuple, w, args):
    ...
    v = tuple(grad1 - grad2 for grad1,
              grad2 in zip(res_tuple[1], res_tuple[2]))
    h_estimate = tuple(grad1 - grad2 for grad1,
                       grad2 in zip(res_tuple[1], res_tuple[2]))
    for _ in range(iteration):
        hv = hvps(res_tuple[0], w, h_estimate)
        with torch.no_grad():
    ...
    #! Gaussian Noise will be added when training since we will remove multiple times and only add noise once. We use args.std to control the noise level. mean = 0 by default.
    # gaussian_noise = [(torch.randn(item.size()) * args.gaussian_std +
    #                    args.gaussian_mean).to(item.device) for item in params_esti]
    # params_esti = [item1 + item2 for item1,
    #                item2 in zip(gaussian_noise, params_esti)]

    return params_change[0]  # only one element in the list
...
def lr_optimize(X, y, lam, b=None, num_steps=100, tol=1e-32, verbose=False, opt_choice='LBFGS', lr=0.01, wd=0, X_val=None, y_val=None):
    ...
    '''
    w = torch.autograd.Variable(torch.zeros(
        X.size(1)).float().to(device), requires_grad=True)
    ...
    if opt_choice == 'LBFGS':
        optimizer = optim.LBFGS(
            [w], lr=lr, tolerance_grad=tol, tolerance_change=1e-32)
    ...
    for i in range(num_steps):
        ...
        if verbose:
            print('Iteration %d: loss = %.6f, grad_norm = %.6f' %
        ...
        if X_val is not None:
            ...
            if verbose:
                print('Val accuracy = %.4f' %
                      val_acc, 'Best Val acc = %.4f' % best_val_acc)
...
def ovr_lr_optimize(X, y, lam, weight=None, b=None, num_steps=100, tol=1e-32, verbose=False, opt_choice='LBFGS', lr=0.01, wd=0, X_val=None, y_val=None, retain_grad=False):
    '''
    y: (n_train, c). one-hot
    ...
    # zero initialization
    w = torch.autograd.Variable(torch.zeros(
        X.size(1), y.size(1)).float().to(device), requires_grad=True)
...
def get_balance_train_mask(y_train, num_classes):
    ...
    for i in range(num_classes):
        if int(C_size_exceed[i]) > 0:
            pick = np.random.choice(All_train_id[y_train == i], int(
                C_size_exceed[i]), replace=False)
            train_balance_mask[pick] = 0
    return train_balance_mask.type(torch.BoolTensor)


def preprocess_data(X, axis_num=1):
    """
    input:
        X: (n,d), torch.Tensor
    """
    ...
    X_norm = norm(X_scaled, axis=axis_num)
